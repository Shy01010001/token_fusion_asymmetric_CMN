...
class CaptionModel(nn.Module):
    ...
    def beam_search(self, init_state, init_logprobs, *args, **kwargs):
        ...
        # print('init_logprobs',init_logprobs)
        # print('init_logprobs',init_logprobs.size())
        # exit()
        ...
        # do one step of classical beam search
        def beam_step(logprobs, unaug_logprobs, beam_size, t, beam_seq, beam_seq_logprobs, beam_logprobs_sum, state):
            ...
            state_ix = (beam_ix + torch.arange(batch_size).type_as(beam_ix).unsqueeze(-1)
        ...
        beam_seq_logprobs_table = [torch.FloatTensor(batch_size, bdash, 0, self.vocab_size + 1).to(device)
        ...
        if self.__class__.__name__ == 'AttEnsemble':
            args = [[[args[j][i][k] for i in range(len(self.models))] for j in range(len(args))]
                    for k in range(group_size)] # group_name, arg_name, model_name
        else:
        ...
        for t in range(self.max_seq_length + group_size - 1):
            for divm in range(group_size):
                if t >= divm and t <= self.max_seq_length + divm - 1:
                    ...
                    state_table[divm] = beam_step(logprobs, unaug_logprobs, bdash, t - divm,
    ...
    def old_beam_search(self, init_state, init_logprobs, *args, **kwargs):
        ...
        beam_seq_logprobs_table = [torch.FloatTensor(self.max_seq_length, bdash, self.vocab_size + 1).zero_()
                                   for _ in range(group_size)]
        ...
        for t in range(self.max_seq_length + group_size - 1):
            for divm in range(group_size):
                if t >= divm and t <= self.max_seq_length + divm - 1:
                    ...
                    beam_logprobs_sum_table[divm], \
                    state_table[divm], \
                    candidates_divm = beam_step(logprobsf, unaug_logprobsf, bdash, t - divm,
                                                beam_seq_table[divm], beam_seq_logprobs_table[divm],
                                                beam_logprobs_sum_table[divm], state_table[divm])
